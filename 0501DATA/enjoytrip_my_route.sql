...
CREATE TABLE `my_route` (
  ...
  `scrap_cnt` int NOT NULL DEFAULT '1',
  PRIMARY KEY (`no`),
  KEY `my_route_to_member_member_no_idx` (`member_no`),
  CONSTRAINT `my_route_to_member_member_no` FOREIGN KEY (`member_no`) REFERENCES `enjoytrip_erd`.`member` (`no`)
